...
namespace Satisfactory3DMap {
    ...
    template<typename T>
    inline s::TVector<T> ueVectorCast(const glm::vec3& vec) {
        return {static_cast<T>(vec.x), static_cast<T>(vec.y), static_cast<T>(vec.z)};
    }

    template<typename T>
    inline s::TQuat<T> ueQuatCast(const glm::quat& quat) {
